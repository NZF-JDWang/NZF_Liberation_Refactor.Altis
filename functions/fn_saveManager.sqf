/*
    Function: KPLIB_fnc_saveManager
    
    Description:
        Handles loading, initialization, and periodic saving of mission state.
        Uses CBA functions for non-blocking operation instead of sleep loops.
    
    Parameters:
        None
    
    Returns:
        Save system initialized [BOOL]
...
// Setup auto save when last player exits
...
// Start the periodic save using CBA_fnc_addPerFrameHandler
private _saveHandler = [{
    if (GRLIB_endgame == 1) then {
        // Exit the PFH and wipe save, if campaign ended
        profileNamespace setVariable [GRLIB_save_key, nil];
        saveProfileNamespace;
        ["Campaign ended. Save data wiped.", "SAVE"] call KPLIB_fnc_log;
        ...
        if (KP_liberation_savegame_debug > 0) then {
            [format ["Campaign saved - Time needed: %1 seconds", diag_tickTime - _start], "SAVE"] call KPLIB_fnc_log;
        };
    };
}, KP_liberation_save_interval, []] call CBA_fnc_addPerFrameHandler;
